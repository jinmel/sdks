...
packages; leave a deprecation notice that points users to the maintained replacement when one
exists, or to `@morpho-org/morpho-sdk` and current Morpho documentation when there is no direct
package successor.

At the time of this TIB, the `@morpho-labs/*` npm deprecation set is:

- `@morpho-labs/ethers-fallback-provider`
- `@morpho-labs/ethers-multicall`
- `@morpho-labs/ethers-utils`
- `@morpho-labs/gitbook-cli`
...
- **Phase 7 -- Compatibility and deprecation:** Publish final compatibility releases or
  ...
  workspace packages after downstream migration. Deprecate every package in the `@morpho-labs/*`
  npm deprecation scope with replacement-specific guidance.
...
- Published packages do not bundle internal dependencies; they publish dependency ranges. A
  ...
  still need updating.
- npm publish permissions are required for compatibility releases and deprecation notices across
  both the `@morpho-org` and `@morpho-labs` npm scopes.
...
- npm download trends for deprecated packages, including the legacy `@morpho-labs/*` packages,
  versus their replacement packages;
